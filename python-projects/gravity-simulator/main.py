...
class planet:
	...
	def draw(self, scale, xoffset, yoffset):
		global windowHeight, windowWidth, orbitTraceLength, distancePerOrbitSubdivide, window
		...
		if planetDrawScale > 1:
			pygame.draw.circle(window, self.color, (int(xoffsetPos), int(yoffsetPos)), int(planetDrawScale))
...
def loadPlanets():
	...
	planetInfos = planet_systems.solar_system
	for planetInfo in planetInfos:
		if planetInfo['relativeTo'] == "None":
			...
			for secondPlanet in planetInfos:
...
def getScientificNotation(num):
    return "{:.2E}".format(num)#Decimal(str(num)))
...
running = True
async def main():
	global dt, frameCount, calculationCycleLimit, pastTime, frameCount, startPauseTimer, paused, dayCounter, yearCounter, timeScale, pastDrawTime, timeBetweenDraw, pastSecond, currentFPS, drawFrameCount, pastOrbitSub, timePerOrbitSubdivide, plantes, scale, yoffset, xoffset, targetScale, selectedPlanet, offsetEase, window, windowWidth, windowHeight, white, frameCount, drawFrameCount
	while running:
